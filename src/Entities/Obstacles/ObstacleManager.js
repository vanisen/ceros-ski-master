...
export class ObstacleManager {
    ...
    placeNewObstacle(gameWindow, previousGameWindow, difficulty) {
        const adjusted_obstacle_chance = Math.floor(NEW_OBSTACLE_CHANCE * (1 - (difficulty - 1) / 10));
        const shouldPlaceObstacle = randomInt(1, adjusted_obstacle_chance);

        if (shouldPlaceObstacle !== adjusted_obstacle_chance) {
            return;
        }
